...
export const saveAgent: RequestHandler = async (req, res) => {
  try {
    ...
    if (!parseResult.success) {
      logger.warn('[Agent] Validation failed:', parseResult.error)
      return res.status(400).json({ success: false, message: 'Invalid agent data', errors: parseResult.error.errors })
    }
    ...
    logger.error('[Agent] Save error:', error)
