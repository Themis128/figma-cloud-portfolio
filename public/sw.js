...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    const options = {
      ...
      vibrate: data.vibrate || [125, 75, 125, 275, 200, 275, 125, 75, 125, 275, 200, 600],
      timestamp: Date.now(),
    ...
    event.waitUntil(
      self.registration.showNotification(data.title || 'Portfolio Notification', options)
        .then(() => {
...
self.addEventListener('notificationclick', (event) => {
  ...
  const targetUrl = action === 'view' ? (data.url || '/') : 
    ...
    clients
      ...
      .then((clientList) => {
        ...
        for (const client of clientList) {
          if (client.url && new URL(client.url).origin === targetOrigin && 'focus' in client) {
    ...
    switch (event.data.type) {
      ...
      case 'CLEAR_CACHE':
        clearSpecificCache(event.data.cacheName).then(result => {
          if (event.ports[0]) {
            event.ports[0].postMessage({ type: 'CACHE_CLEARED', success: result })
          }
        })
        break
...
async function reportCacheStats() {
  try {
    ...
    for (const cacheName of cacheNames) {
      ...
      let totalSize = 0
      for (const request of requests.slice(0, 5)) { // Sample first 5
        try {
