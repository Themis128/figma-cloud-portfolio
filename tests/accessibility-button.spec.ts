...
test('Accessibility button opens settings', async ({ page }) => {
  ...
  await expect(button).toBeVisible({ timeout: 40000 })
  await button.waitFor({ state: 'visible', timeout: 40000 })
  await button.click()
  const panel = page.getByTestId('accessibility-panel')
  await panel.waitFor({ state: 'visible', timeout: 40000 })
  await expect(panel).toBeVisible({ timeout: 40000 })
  const closeButton = page.locator('button[aria-label="Close"]')
  await closeButton.waitFor({ state: 'visible', timeout: 40000 })
  await closeButton.click()
  await panel.waitFor({ state: 'hidden', timeout: 40000 })
  await expect(panel).toBeHidden({ timeout: 40000 })
})
