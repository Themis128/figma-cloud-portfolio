...
test.describe("Baltzakis Themistoklis Portfolio", () => {
  ...
  test("should load the main page with comprehensive performance metrics", async ({
    ...
    // Wait for React to hydrate (waitForAppReady already ran; brief additional pause is sufficient)
    await page.waitForTimeout(2000);

    // Basic checks - verify HTML structure is correct
    console.log("Checking basic HTML structure...");

    // Check if the app root div exists (Next.js uses #__next, fall back to body)
    const appRoot = page.locator("#__next, body");
    await expect(appRoot.first()).toBeAttached();
    console.log("App root div exists");
    ...
    // Check for images if present (home page may use SVG instead of <img>)
    const images = page.locator("img");
    const imageCount = await images.count();

    // Verify all images have alt text (if any images exist)
    if (imageCount > 0) {
  ...
  test("should display main navigation links", async ({ page }) => {
    ...
    if (viewportSize && viewportSize.width >= 768) {
      ...
      await expect(
        page.getByRole("link", { name: "Performance" }).first(),
      ).toBeVisible();
      await expect(
  ...
  test("should have skip link for accessibility", async ({ page }) => {
    ...
    // Check for skip to main content link (sr-only by default, visible on focus)
  ...
  test("should close mobile menu on navigation", async ({ page }) => {
    ...
    // Mobile menu button should indicate open state via aria-expanded
    await expect(mobileMenuButton).toHaveAttribute("aria-expanded", "true");

    // Wait for CSS transition to complete
    await page.waitForTimeout(500);

    // Click a navigation link in the mobile menu container
    const mobileMenuContainer = page.locator(".md\\:hidden.absolute");
    await mobileMenuContainer.locator("a").first().click();

    // Mobile menu should be closed - button should indicate collapsed state
  ...
  test("should have proper ARIA labels", async ({ page }) => {
    ...
    // Check that accessibility button has aria-label
    const accessibilityButton = page.locator('[data-testid="accessibility-toggle"]');
    await expect(accessibilityButton).toBeAttached();
    await expect(accessibilityButton).toHaveAttribute("aria-label", "Open accessibility settings");
  ...
  test("should have comprehensive accessibility features", async ({ page }) => {
    ...
    // Check for proper form labels (skip hidden inputs — they don't need visible labels)
    ...
    if (inputCount > 0) {
      for (let i = 0; i < inputCount; i++) {
        ...
        // Each input should have either an id, aria-label, aria-labelledby, or placeholder
  ...
  test("should handle navigation between pages", async ({ page }) => {
    ...
    if (await contactLink.isVisible({ timeout: 2000 })) {
      ...
      const isContactHeadingVisible = await contactHeading
        .isVisible()
        .catch(() => false);
      const isGetInTouchVisible = await getInTouchHeading
        .isVisible()
        .catch(() => false);

      // At least one of these should be visible
      expect(
        isFormVisible || isContactHeadingVisible || isGetInTouchVisible,
      ).toBe(true);
    }
  });
  ...
  test.describe("Contact Form", () => {
    ...
    test("should display contact form with all required fields", async ({
      ...
      // All visible fields have the required attribute (verified in the next test)
    });
    ...
    test("should handle form submission error", async ({ page }) => {
      // Mock a network error by intercepting the form submission API
    ...
    test("should handle keyboard navigation in form", async ({ page }) => {
      ...
      // Tab to the submit button — may need multiple tabs if there are intermediate focusable elements
      // Try pressing Tab up to 3 times to reach the submit button
      let submitFocused = false;
      const submitButton = page.locator('button[type="submit"]');
      for (let i = 0; i < 3; i++) {
        await page.keyboard.press("Tab");
        submitFocused = await submitButton.evaluate(
          (el) => el === document.activeElement || el.contains(document.activeElement),
        );
        if (submitFocused) break;
      }
      // Verify submit button is reachable via keyboard
      expect(submitFocused).toBe(true);
    });

    test("should display contact information correctly", async ({ page }) => {
      // Check contact details section heading
      await expect(
        page.getByRole("heading", { name: "Get In Touch" }).first(),
      ).toBeVisible();

      // Check location
      await expect(page.getByText("Koropi/Athens, Greece").first()).toBeVisible();

      // Check phone
      await expect(page.getByText("+30 697 777 7838").first()).toBeVisible();

      // Check email
      await expect(page.getByText("baltzakis.themis@gmail.com").first()).toBeVisible();

      // Check LinkedIn link
      const linkedinLink = page.locator('a[href="https://linkedin.com/in/baltzakis-themis"]').first();
      await expect(linkedinLink).toBeAttached();

      // Check portfolio link (actual href in the codebase)
      const portfolioLink = page.locator('a[href="https://baltzakisthemis.com"]').first();
  ...
  test.describe("Theme Switcher", () => {
    ...
    test("should update meta theme-color for mobile browsers", async ({
      ...
      await page.goto("/");
      await waitForAppReady(page);
      // Check meta theme-color exists (Next.js may render multiple for light/dark media queries)
      const themeMetas = page.locator('meta[name="theme-color"]');
      const count = await themeMetas.count();
      expect(count).toBeGreaterThan(0);

      // Just verify the first meta theme-color has a content value
      const content = await themeMetas.first().getAttribute("content");
  ...
  test.describe("Settings Page", () => {
    ...
    test("should load settings page with all sections", async ({ page }) => {
      ...
      await expect(page.getByText("Customize your portfolio experience.")).toBeVisible();
    ...
    test("should handle theme settings", async ({ page }) => {
      // Check theme options (these are buttons with aria-pressed, not label inputs)
      await expect(page.getByRole("button", { name: "Light" })).toBeVisible();
      await expect(page.getByRole("button", { name: "Dark" })).toBeVisible();
      await expect(page.getByRole("button", { name: "System" })).toBeVisible();

      // Test theme switching - click Dark button
    ...
    test("should handle privacy settings", async ({ page }) => {
      // Check privacy section exists with Analytics toggle
      ...
      // Check the analytics toggle switch
  ...
  test.describe("Performance Page", () => {
    ...
    test("should load performance page with heading", async ({ page }) => {
      // Check that the performance page loads with the correct title
      await expect(
        page.locator("h1").filter({ hasText: "Performance" }),
      ).toBeVisible();

      // Check for live measurement subtitle
      await expect(page.getByText(/Measured live/i)).toBeVisible();
    });
    ...
    test("should display performance sections", async ({ page }) => {
      // Check that the performance page loads with the correct heading
      await expect(
        page.locator("h1").filter({ hasText: "Performance" }),
      ).toBeVisible();
      ...
      // Check for performance-related content on the page (Core Web Vitals, metrics, etc.)
      const hasPerformanceContent = await page
        .locator("text=/LCP|CLS|FCP|TTFB|Core Web Vitals|Speed Test|Optimization/i")
        .count();
    ...
    test("should display optimization status", async ({ page }) => {
      ...
      // Verify page has some text content (main assertion)
  ...
  test.describe("Navigation Component", () => {
    test("should handle mobile navigation menu", async ({ page }) => {
      ...
      // Look for mobile menu button using aria-label
      const mobileMenuButton = page.getByRole("button", {
        name: "Toggle menu",
      });
      ...
      if (await mobileMenuButton.isVisible()) {
        ...
        // Check menu is open via aria-expanded
        await expect(mobileMenuButton).toHaveAttribute("aria-expanded", "true");

        // Check that menu links are visible
        const mobileMenuContainer = page.locator(".md\\:hidden.absolute");
        const links = mobileMenuContainer.locator("a");
