  ...
  test.describe('GitHub Workflows API', () => {
    ...
    test('should fetch workflow runs', async ({ page }) => {
      ...
      const runs = await page.evaluate(async () => {
        try {
          const response = await fetch('http://localhost:3000/api/github/runs/1')
          return await response.json()
        } catch (_error) {
          return null
        }
    ...
    test('should fetch job details', async ({ page }) => {
      ...
      const jobs = await page.evaluate(async () => {
        try {
          const response = await fetch('http://localhost:3000/api/github/jobs/12345')
          return await response.json()
        } catch (_error) {
          return null
        }
    ...
    test('should handle GitHub API authentication', async ({ page }) => {
      ...
      const response = await page.evaluate(async () => {
        try {
          const res = await fetch('http://localhost:3000/api/github/workflows/test-repo')
          return {
            status: res.status,
  ...
  test.describe('LRU Cache Implementation', () => {
    test('should cache GitHub API responses', async ({ page }) => {
      ...
      await page.evaluate(async () => {
        ...
        await fetch(`http://localhost:3000/api/github/workflows/${repo}`)
        await fetch(`http://localhost:3000/api/github/workflows/${repo}`)
        await fetch(`http://localhost:3000/api/github/workflows/${repo}`)
      })
  ...
  test.describe('Error Handling and Resilience', () => {
    test('should handle network errors', async ({ page }) => {
      ...
      const networkError = await page.evaluate(async () => {
        try {
          await fetch('http://localhost:3000/api/github/workflows/test-repo')
          return false
        } catch (_error) {
          return true
        }
    ...
    test('should handle GitHub API timeouts', async ({ page }) => {
      ...
      const timeoutTest = await page.evaluate(async () => {
        try {
          ...
          const response = await fetch('http://localhost:3000/api/github/workflows/test-repo', {
    ...
    test('should provide fallback data on API failure', async ({ page }) => {
      ...
      const fallbackTest = await page.evaluate(() => {
        ...
        return fetchWithFallback('http://localhost:3000/api/github/workflows/test-repo', {
          workflows: [],
          total_count: 0,
          cached: true,
        })
