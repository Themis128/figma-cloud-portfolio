  ...
  test.describe("DNS Prefetching", () => {
    ...
    test("should have DNS prefetch links", async ({ page }) => {
      ...
      if ((await dnsPrefetchLinks.count()) > 0) {
        ...
        console.log(
          "No DNS prefetch links found - may not be needed for this page",
        );
      }
    });
    ...
    test("should use modern image formats", async ({ page }) => {
      ...
      for (const img of await images.all()) {
        ...
        if (src && !src.startsWith("data:")) {
          ...
          expect(usesModernFormat || src.match(/\.(jpg|jpeg|png|gif)$/)).toBe(
            true,
          );
        }
      }
          ...
          setTimeout(() => {
            const fontEntries = performance
              ...
              .filter(
                (entry) =>
                  entry.name.includes("font") ||
                  entry.name.match(/\.(woff2?|ttf|otf)$/),
              ) as PerformanceResourceTiming[];
  ...
  test.describe("Bundle Optimization", () => {
    test("should split code bundles effectively", async ({ page }) => {
      ...
      if (jsScripts.length === 0) {
        console.log(
          "No JS scripts found - this may be normal in development mode",
        );
      ...
      if (jsScripts.length === 1) {
        console.log(
          "Single bundle detected - this is normal in development mode",
        );
      }
    ...
    test("should cache static assets", async ({ page }) => {
      ...
      for (const headers of cacheHeaders) {
        expect(
          headers.cacheControl || headers.etag || headers.lastModified,
        ).toBeTruthy();
      }
    });
  });
