  ...
  analyzeFailure(test, result) {
    ...
    if (errorMessage.includes('Timeout') || errorMessage.includes('timeout')) {
      suggestions.push({
        type: 'timeout',
        test: testId,
        suggestion: 'Consider increasing timeout or checking for infinite loading states',
    ...
    if (errorMessage.includes('stale') || errorMessage.includes(' detached')) {
      suggestions.push({
        type: 'stale',
        test: testId,
        suggestion: 'Element was detached from DOM (common with React re-renders)',
  ...
  generateRecommendations() {
    ...
    if (
      ...
      recommendations.push({
        ...
        action: 'Autofix is working well. Consider enabling more autofix options.',
      })
    }
