...
test.describe("Autofix Feature Demonstration", () => {
  test("should demonstrate locator healing on stale elements", async ({
    page,
  }) => {
    await page.goto(
    ...
    // Verify the button was clickable (healing succeeded - attempts may be 0 on clean success)
    // The locator re-queries the DOM on each attempt, so a fresh element is found successfully
    expect(healingBtn.getHealAttempts()).toBeGreaterThanOrEqual(0);
  });

  test("should demonstrate smart waiting for dynamic content", async ({
    page,
  }) => {
    await page.goto(
  ...
  test("should demonstrate selector optimization suggestions", async ({
    page,
  }) => {
    await page.goto(
  ...
  test("should handle locator failures gracefully", async ({ page }) => {
    await page.goto(
      "data:text/html,<html><body><div>Content</div></body></html>",
    );
    ...
    // This should fail gracefully after retries — use a short timeout to avoid hanging
    await expect(healingLocator.click({ timeout: 1000 })).rejects.toThrow();
    // The attempts counter is set to the failed attempt number (>= 1)
