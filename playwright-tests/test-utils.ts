...
/**
 * Wait for the Next.js app to be fully loaded and ready
 */
export async function waitForAppReady(page: Page) {
  try {
    ...
    // Wait for Next.js app to be ready - look for common Next.js elements
    await page.waitForFunction(
      () => {
        // Check for Next.js app container or main content
        const appContainer =
        ...
        // Check if the app has content (not just loading state)
        const textContent = appContainer.textContent || "";
        const hasContent = textContent.trim().length > 10;

        // Check if any interactive elements are present
        const hasInteractiveElements =
          document.querySelector("button, a, input, form") !== null;
      ...
      .waitForFunction(
        () => {
          ...
          return (
            ...
            mainContent.textContent.trim().length > 5
          );
        },
        { timeout: 10000 },
      )
    ...
    try {
      ...
      if (stable) {
        ...
        if (initialBox && finalBox) {
          const movement =
            Math.abs(initialBox.x - finalBox.x) +
 ...
 */
export async function handleNetworkFlakiness(
  page: Page,
  _context: BrowserContext,
) {
  ...
  try {
    ...
    for (const fallback of fallbacks) {
      try {
        ...
        await element.waitFor({ state: "visible", timeout: 2000 });
        console.log(
          `🔄 Used fallback selector: ${fallback} instead of ${primarySelector}`,
        );
        return element;
      } catch (_error) {
...
export function generateTestRecommendations(results: {
  ...
  if (results.failed > 0) {
    recommendations.push(
      "🔴 Some tests failed - check screenshots and traces for details",
    );
  }
  ...
  if (results.duration > 300000) {
    // 5 minutes
    recommendations.push(
      "⏱️  Tests are running slowly - consider optimizing or parallelizing",
    );
  }
