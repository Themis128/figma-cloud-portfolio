...
test.describe("Comprehensive reCAPTCHA and Google Analytics Integration Tests", () => {
  ...
  test.describe.skip("reCAPTCHA v3 Server-Side Validation (requires external API (API_BASE_URL))", () => {
    ...
    test("should require all mandatory fields", async ({ request }) => {
      const testCases = [
        { name: "Test User", email: "test@example.com", subject: "Test Subject", recaptchaToken: "test-token" },
        { name: "Test User", email: "test@example.com", message: "Test message", recaptchaToken: "test-token" },
        { name: "Test User", subject: "Test Subject", message: "Test message", recaptchaToken: "test-token" },
