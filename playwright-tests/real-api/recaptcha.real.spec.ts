...
test.describe("Google reCAPTCHA v3 - Real Integration", () => {
  ...
  test("should load real reCAPTCHA v3 script", async ({ page }) => {
    ...
    const { result: captchaStatus, duration } = await measureAPICall(
      ...
      async () => {
        ...
        return await page.evaluate(() => {
          ...
          const hasGrecaptcha = !!(
            window as typeof window & { grecaptcha?: unknown }
          ).grecaptcha;
    ...
    console.log(
      `  grecaptcha Object: ${captchaStatus.grecaptchaLoaded ? "✅" : "❌"}`,
    );
    ...
    expect(
      captchaStatus.grecaptchaLoaded || captchaStatus.scriptsFound > 0,
    ).toBe(true);
  });
  ...
  test("should verify real token with backend", async ({ page }) => {
    ...
    if (verification.score !== undefined) {
      console.log(
        `  reCAPTCHA Score: ${verification.score} (0.0 = bot, 1.0 = human)`,
      );
    }
  ...
  test("should handle contact form submission with real CAPTCHA", async ({
    page,
  }) => {
    await setupRealAPIPage(page);
    await page.goto("/contact");
  ...
  test("should validate reCAPTCHA integration best practices", async ({
    page,
  }) => {
    await setupRealAPIPage(page);
    await page.goto("/contact");
      ...
      async () => {
        ...
        return await page.evaluate(() => {
          const checks = {
            ...
            noExplicitButton: !document.querySelector(
              'input[type="checkbox"].g-recaptcha',
            ),
            scriptLoadedAsync: Array.from(
              document.querySelectorAll("script[src*='recaptcha']"),
            ).some((script) => (script as HTMLScriptElement).async),
            privacyTermsVisible: !!document.querySelector(
              'a[href*="recaptcha/terms"]',
            ),
          };
    ...
    console.log(
      `  Invisible Mode (v3): ${bestPractices.invisibleMode ? "✅" : "❌"}`,
    );
    ...
    console.log(
      `  Async Loading: ${bestPractices.scriptLoadedAsync ? "✅" : "⚠️"}`,
    );
