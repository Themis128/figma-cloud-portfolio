...
test.describe("Firebase - Real Integration", () => {
  ...
  test("should handle push notification subscription flow", async ({
    page,
    context,
  }) => {
    await setupRealAPIPage(page);
    ...
    if (!buttonExists) {
      console.log(
        "⏭️  Notification button not found - skipping subscription test",
      );
      test.skip();
      return;
    }
    ...
    console.log(
      `📬 Subscription Attempted: ${subscriptionAttempted ? "✅" : "❌"}`,
    );
