...
test.describe("Socket.IO - Real Integration", () => {
  ...
  test("should establish real WebSocket connection", async ({ page }) => {
      ...
      async () => {
        return await page.evaluate(() => {
          return new Promise((resolve) => {
            ...
            const hasSocketIO = !!(window as typeof window & { io?: unknown })
              .io;
            ...
            try {
              const socket = (
                window as typeof window & { io?: (url: string) => unknown }
    ...
    console.log(
      `  Socket.IO Available: ${connectionStatus.available ? "✅" : "❌"}`,
    );
    ...
    if (connectionStatus.connected) {
      ...
      console.log(
        `  💡 Make sure backend server is running: npx tsx server/node-build.ts`,
      );
    }
  ...
  test("should send and receive real messages", async ({ page }) => {
    ...
    const { result: messageTest, duration } = await measureAPICall(
      "Message Exchange",
      async () => {
        return await page.evaluate(() => {
          return new Promise((resolve) => {
  ...
  test("should join and leave real rooms", async ({ page }) => {
    ...
    const { result: roomTest, duration } = await measureAPICall(
      "Room Management",
      async () => {
        return await page.evaluate(() => {
          return new Promise((resolve) => {
  ...
  test("should handle real connection drops and reconnection", async ({
    page,
  }) => {
    await setupRealAPIPage(page);
    await page.goto("http://localhost:3001/");
  ...
  test("should verify backend server availability", async ({ page }) => {
    ...
    if (!serverCheck.available) {
      console.log(
        `  ⚠️  Backend not running. Start with: npx tsx server/node-build.ts`,
      );
    }
