...
test.describe("GitHub API - Real Integration", () => {
  ...
  test("should fetch real GitHub workflows", async ({ page }) => {
    ...
    const { result: workflows, duration } = await measureAPICall(
      "GitHub Workflows",
      async () => {
  ...
  test("should fetch real workflow runs", async ({ page }) => {
    ...
    const { result: runs, duration } = await measureAPICall(
    ...
    if (runs.workflow_runs.length > 0) {
      ...
      expect(["queued", "in_progress", "completed"]).toContain(
        latestRun.status,
      );
    }
  ...
  test("should verify LRU cache effectiveness with real API", async ({
    page,
  }) => {
    await setupRealAPIPage(page);
    ...
    if (cacheImprovement > 0) {
      console.log(
        `  ✅ Cache improved performance by ${cacheImprovement.toFixed(1)}%`,
      );
    }
  });
  ...
  test("should handle GitHub API authentication errors", async ({ page }) => {
    ...
    console.log(
      `✅ Authentication error handled correctly: ${response.status()}`,
    );
  });
