...
test.describe("Google Analytics 4 - Real Integration", () => {
    ...
    const { result: gaStatus, duration } = await measureAPICall(
      ...
      async () => {
        ...
        return await page.evaluate(() => {
          ...
          const hasGtag =
            typeof (window as typeof window & { gtag?: unknown }).gtag ===
            "function";
  ...
  test("should send real custom event", async ({ page }) => {
    ...
    if (customEvent.sent) {
      console.log(
        `  Timestamp: ${new Date(customEvent.timestamp).toLocaleString()}`,
      );
