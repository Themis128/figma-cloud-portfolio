...
export function validateAPICredentials(
  ...
  if (missing.length > 0) {
    console.warn(
      `⚠️  ${apiName}: Missing required environment variables: ${missing.join(", ")}`,
    );
    return false;
  }
...
export async function cleanupTestData(
  ...
  if (process.env.CLEANUP_TEST_DATA !== "true") {
    console.log(
      `⏭️  Skipping cleanup for ${apiName} (CLEANUP_TEST_DATA=false)`,
    );
    return;
  }
...
export async function checkAPIAvailability(
  ...
  try {
    ...
    console.log(
      `${available ? "✅" : "❌"} ${apiName} availability: ${response.status}`,
    );
    return available;
  } catch (error) {
...
export class APIUsageTracker {
  ...
  getReport(): string {
    ...
    this.metrics.forEach((metrics, apiName) => {
      const avgDuration =
        metrics.calls > 0 ? metrics.totalDuration / metrics.calls : 0;
      const successRate =
        metrics.calls > 0 ? (1 - metrics.errors / metrics.calls) * 100 : 0;
