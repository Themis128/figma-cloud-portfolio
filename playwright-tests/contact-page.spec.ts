...
test.describe("Contact Page — Section Navigation", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/contact/");
    await page.waitForLoadState("domcontentloaded");
  });
  ...
  test("should have all section IDs for navigation", async ({ page }) => {
    const sectionIds = ["hero", "contact-info", "contact-form", "more-info"];
  ...
  test("should display sticky section nav on desktop", async ({ page }) => {
    ...
    const sectionNav = page.locator('nav[aria-label="Contact page sections"]');
