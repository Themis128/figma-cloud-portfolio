...
test.describe('Product / Work Experience Page', () => {
  ...
  test('renders at least one experience card', async ({ page }) => {
    ...
    const cards = page.locator('[class*="card"], article, [class*="experience"], li')
  ...
  test('displays employment period', async ({ page }) => {
    const period = page.getByText('December 2024', { exact: false })
  ...
  test('renders responsibilities list items', async ({ page }) => {
    // Responsibilities are rendered as list items
    const listItems = page.locator('li, [class*="responsibility"]')
    const count = await listItems.count()
    expect(count).toBeGreaterThan(0)
  })

  test('responsibilities contain meaningful technical content', async ({ page }) => {
  ...
  test('shows building/company icon or indicator', async ({ page }) => {
    // Building icon from lucide-react (rendered as SVG)
  ...
  test('shows calendar/date information', async ({ page }) => {
    ...
    const dateIndicator = page.locator('svg, [class*="calendar"], [class*="date"]').first()
    await expect(dateIndicator).toBeVisible()
  })

  test('shows location icon or text', async ({ page }) => {
    const locationIndicator = page.locator('svg, [class*="location"], [class*="map"]').first()
  ...
  test('navigation home link works (SPA routing)', async ({ page }) => {
    // Click the home/logo link in navigation
  ...
  test('circuit background or decorative background renders', async ({ page }) => {
    // CircuitBackground renders SVG or canvas
    const bg = page.locator('canvas, svg, [class*="circuit"], [class*="background"]')
  ...
  test('page has no "undefined" or "[object Object]" text rendered', async ({ page }) => {
    const bodyText = await page.locator('body').textContent()
    expect(bodyText).not.toContain('[object Object]')
    expect(bodyText).not.toContain('undefined')
  })

  test('page has substantial content (more than 200 characters)', async ({ page }) => {
    const bodyText = await page.locator('body').textContent()
