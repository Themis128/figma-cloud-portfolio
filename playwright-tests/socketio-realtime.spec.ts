...
test.describe("Socket.IO Real time Features", () => {
    ...
    test("should track connected users", async ({ page }) => {
      ...
      await page.evaluate(() => {
        window.presenceTracker = {
          ...
          addUser: function (
            userId: string,
            userData: { name: string; status: string },
          ) {
  ...
  test.describe("Typing Indicators", () => {
    ...
    test("should auto-clear typing indicator after timeout", async ({
      page,
    }) => {
      await page.goto("/");
      await waitForAppReady(page);
    ...
    test("should broadcast messages to room", async ({ page }) => {
      ...
      const broadcastTest = await page.evaluate(() => {
        if (window.messageBroadcast) {
          ...
          return {
            room1Messages:
              window.messageBroadcast.getMessagesForRoom("room1").length,
            room2Messages:
              window.messageBroadcast.getMessagesForRoom("room2").length,
