...
steps:
  ...
  - name: Run Playwright tests
    run: |
      echo "Running Playwright E2E tests (chromium only)..."
      npx playwright test --config=playwright.config.ci.ts \
        --project=chromium \
        --reporter=json,list \
        2>&1 | tee /tmp/playwright-results.txt || true
      # Extract JSON report if generated by CI config
      if [ -f playwright-report/results.json ]; then
        cp playwright-report/results.json /tmp/playwright-report.json
      fi
