...
# Production Lambda URLs (auto-generated by Amplify)
...
- **Development**: Uses local Express server (`/api/*` routes)
- **Production**: Uses AWS Lambda functions (configured via `VITE_LAMBDA_*` variables)
...
```tsx
// Toast notifications using Sonner + custom hook
import { useToast } from "@/hooks/use-toast";
...
customToast({
  ...
  action: <Button onClick={handleAction}>View</Button>,
});
```
...
className="text-responsive-xl btn-mobile card-mobile"
```
...
import { messaging } from "@/lib/aws-notifications";
...
pnpm test:e2e               # Run all browsers (chromium, firefox, webkit)
pnpm test:e2e:ui            # Interactive mode with Playwright UI
pnpm test:e2e:fast          # Fast configuration (chromium only, fewer tests)
pnpm test:e2e:critical      # Critical/smoke tests only
pnpm test:e2e:real          # Real API tests (requires live services)
pnpm test:e2e:dashboard     # With test dashboard UI
pnpm test:e2e:continuous    # Continuous testing mode
pnpm test:e2e:debug         # Debug mode with browser visible
pnpm test:e2e:ci            # CI configuration (headless, parallel)
```
...
- **Vitest**: Unit tests with jsdom environment, coverage thresholds at 20%
  ...
  - Test globals enabled for easier imports

- **Playwright**: Multi-browser E2E with custom timeouts and retry logic
  - Located in `playwright-tests/` directory
  ...
  - Multiple config files for different scenarios:
    ...
    - `playwright.config.real-api.ts` - Tests with real APIs
    - `playwright.config.ci.ts` - CI-optimized settings

- **Web Server Auto-start**: Both frontend (port 8081) and backend (port 3002) start automatically for E2E tests
...
pnpm coverage:upload     # Upload to Codacy
```
...
3. **Environment Setup**: Copy `.env.example` to `.env` and configure all required variables - or use AWS Secrets Manager

### 📁 Project Structure & Imports
4. **Path Imports**: Always use `@/` and `@shared/` aliases, never relative paths like `../../`
5. **Type Safety**: Define interfaces in `@shared/api.ts` before implementing client/server features
6. **Lazy Loading**: All page components MUST be lazy-loaded with React.lazy() for code splitting

### 🏗️ Build & Deployment
7. **Build Order**: Always run `pnpm run build:resume` before `pnpm run build:client` (PDF templates needed)
...
11. **reCAPTCHA**: Contact form requires valid reCAPTCHA token - test with real API keys
12. **CSP Headers**: Modifying CSP in `vite.config.ts` requires restarting dev server
13. **MCP Configuration**: MCP server settings live in `%APPDATA%` (outside repo) - never commit Cline MCP settings with hardcoded tokens, always use environment variable references like `${GITHUB_TOKEN}`

### 🧪 Testing & Quality
14. **Testing**: Exclude app.spec.ts and logo.spec.ts from Vitest runs (handled by Playwright)
15. **Console Logs**: Biome warns about console statements - remove before production or use structured logging

### 🔄 Real-time & State Management
16. **Socket.IO Rooms**: Always leave rooms when component unmounts to prevent memory leaks
17. **Toast System**: Use `useToast` hook for consistent notifications, `toast` from Sonner for simple messages
18. **Mobile Detection**: Use `useIsMobile()` hook instead of manual media queries for responsive behavior
19. **AWS SNS**: For push notifications, use `messaging.getToken()` from `@/lib/aws-notifications`

### 📚 Documentation & Maintenance
20. **Documentation Updates**: ALWAYS update relevant documentation when adding/changing features:
...
├── lib/                 # Utilities (api.ts, aiService.ts, agentExecutor.ts, aws-notifications.ts, utils.ts)
├── hooks/               # Custom React hooks (usePerformanceMonitoring, useSocket, useToast, etc.)
