    ...
    setInterval(() => {
      ...
      if (
        ...
        memory.usedJSHeapSize >
          memory.jsHeapSizeLimit * MEMORY_USAGE_WARNING_THRESHOLD
      ) {
        // eslint-disable-next-line no-console
        console.warn(
          `[PerformanceOptimizer] High memory usage: ${(memory.usedJSHeapSize / 1_048_576).toFixed(1)}MB used of ${(memory.jsHeapSizeLimit / 1_048_576).toFixed(0)}MB limit`,
        );
      }
      ...
      const longTaskObserver = new PerformanceObserver((list) => {
        ...
        for (const entry of entries) {
          if (entry.duration > LONG_TASK_DURATION_THRESHOLD_MS) {
            // eslint-disable-next-line no-console
            console.warn(
              `[PerformanceOptimizer] Long task detected: ${entry.duration.toFixed(0)}ms`,
            );
          }
        }
      });
