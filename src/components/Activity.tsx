...
export function Activity({
  ...
  const activityId = React.useMemo(
    () => Math.random().toString(ACTIVITY_ID_BASE36_RADIX),
    [],
  );
...
export function ActivityBoundary({
  ...
  return (
    <div
      className={`activity-boundary ${className}`}
      data-activity-boundary={mode}
    >
        ...
        return (
          ...
          >
            {React.isValidElement(child)
              ? React.cloneElement(
                  child as React.ReactElement<{ [key: string]: unknown }>,
                  {
