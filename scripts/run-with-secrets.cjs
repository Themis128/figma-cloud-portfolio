...
if (isWindows) {
  ...
  try {
    ...
    // Build command arguments as a properly escaped PowerShell array
    let argsPart = ''
    if (commandArgs.length > 0) {
      const escapedArgs = commandArgs.map((arg) => {
        // Escape for PowerShell: replace ' with ''
        const escaped = arg.replace(/'/g, "''")
        return `'${escaped}'`
      })
      argsPart = `, ${escapedArgs.join(', ')}`
    }

    // Build the full PowerShell command
    const psCommand = `& "${absoluteScriptPath}" -Command '${command}'${argsPart}`
