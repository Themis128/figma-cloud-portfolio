...
const foundConfigs = []
if (fs.existsSync(projectPnpmrc)) foundConfigs.push({ source: 'project', path: projectPnpmrc })
if (fs.existsSync(userPnpmrc)) foundConfigs.push({ source: 'user', path: userPnpmrc })

if (foundConfigs.length > 0) {
  if (!outputJson) console.log(`✅ .pnpmrc found (${foundConfigs.map((c) => c.source).join(', ')})`)
...
if (fs.existsSync(lockfilePath)) {
  ...
  const KB_SIZE = 1024
  if (!outputJson) console.log(`✅ Lockfile found (${(stats.size / KB_SIZE).toFixed(2)} KB)`)
} else {
...
if (applyChanges) {
  ...
  try {
    ...
    if (toAppend) {
      ...
      if (!outputJson) console.log(`✨ Applied recommended settings to ${target}`)
    } else {
      if (!outputJson) console.log(`✔️  Project .pnpmrc already contains recommended settings`)
    }
  } catch (err) {
