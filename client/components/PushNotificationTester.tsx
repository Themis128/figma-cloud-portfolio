...
export function PushNotificationTester() {
  ...
  const checkServiceWorker = useCallback(async () => {
    if ('serviceWorker' in navigator) {
      try {
        ...
        // Service worker check failed - silently handle
      }
    }
  }, [])

  const isHtmlResponseError = useCallback((errorMessage: string) => {
  ...
  const handleSubscriptionError = useCallback(
    (error: unknown) => {
      ...
      if (isHtmlResponseError(errorMessage)) {
        setResult({
          success: false,
          message:
            'API server may not be running. Please ensure both frontend and backend servers are started.',
        })
      } else {
        setResult({
          success: false,
          message: `Error checking subscriptions: ${errorMessage}`,
        })
      }
    },
    [isHtmlResponseError],
  ...
  const hasActiveSubscriptions = useCallback((subsData: { subscriptions: number }) => {
    return subsData.subscriptions && subsData.subscriptions > 0
  }, [])

  const sendCustomNotificationRequest = useCallback(async () => {
    return await pushNotificationsApi.sendCustomNotification({
      title: customTitle,
      body: customBody,
      icon: '/logo.jpg',
      badge: '/logo.jpg',
  ...
  }, [customTitle, customBody, customUrl])
  ...
  const sendCustomNotification = async () => {
    ...
    try {
      ...
      if (!hasActiveSubscriptions(subsData)) {
        setResult({
          success: false,
      ...
      const data = await sendCustomNotificationRequest()
      setResult({
        success: true,
  ...
  const requestNotificationPermission = async () => {
    if ('Notification' in window) {
      try {
        ...
        // Notification permission request failed - silently handle
      }
    }
  }
  ...
  // Extract render functions to reduce complexity
  ...
  const renderCustomNotificationForm = () => (
  ...
  const renderResults = () => {
    ...
    return (
      <div
        ...
        data-testid={result.success ? 'notification-success' : 'notification-error'}
      >
  ...
  return (
    <Card className='p-6 space-y-6' data-testid='push-notification-tester'>
