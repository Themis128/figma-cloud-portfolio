...
>(
  (
    ...
    const getItemKey = (item: ChartTooltipItem): string => {
      return `${nameKey || item.name || item.dataKey || 'value'}`
    }

    const getItemConfig = (item: ChartTooltipItem): ChartConfigItem | undefined => {
    ...
    const renderTooltipItem = (item: ChartTooltipItem, index: number): React.ReactNode => {
      ...
      return (
        ...
        >
          {renderIndicator(item, itemConfig)}
          {renderItemContent(item, itemConfig)}
        </div>
      )
    }
