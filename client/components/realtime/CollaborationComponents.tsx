...
import type { AgentCollaborationData, RealTimeEvent, UserPresence } from "../types/realtime";
...
export function CollaborationPanel({
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className='flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-navy-750 transition-colors'
...
export function TypingIndicator({ roomId, className }: TypingIndicatorProps) {
  ...
  const typingUsers = useMemo(() => {
    ...
    return events
      .filter(
        (event) =>
          event.metadata?.roomId === roomId && event.data.isTyping && now - event.timestamp < 5000, // 5 second timeout
      )
