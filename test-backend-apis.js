...
async function testBackendAPIs() {
  ...
  try {
    ...
    const contactSuccess = contact.status === 200 || contact.status === 400; // 400 is expected due to invalid recaptcha
    logResult('/api/contact (POST)', contactSuccess, `Status: ${contact.status}, Response: ${JSON.stringify(contact.data)}`);
    ...
    const analyticsSuccess = analytics.status === 200;
    ...
    const resumeSuccess = resume.status === 200 || resume.status === 500; // 500 might be expected if Puppeteer isn't configured
    logResult('/api/resume/download (GET)', resumeSuccess, `Status: ${resume.status}, Content-Type: ${resume.headers['content-type']}`);
    ...
    const vapidSuccess = vapid.status === 200 && vapid.data.publicKey;
    ...
    const subscriptions = await makeRequest(`${baseUrl}/api/push-notifications?action=subscriptions`);
    const subsSuccess = subscriptions.status === 200;
    logResult('/api/push-notifications Subscriptions', subsSuccess, `Status: ${subscriptions.status}`);
    ...
    const workflowsSuccess = workflows.status === 200 || workflows.status === 401 || workflows.status === 403; // Auth required
    logResult('/api/github/workflows', workflowsSuccess, `Status: ${workflows.status} (auth may be required)`);
    ...
    const notFoundSuccess = notFound.status === 404;
    ...
    const manifestSuccess = manifest.status === 200;
