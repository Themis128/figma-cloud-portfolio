...
async function fetchWorkflowData() {
  try {
    ...
    if (response.status === 401) {
      throw new Error(
        "GitHub authentication required. Please provide a valid token.",
      );
    } else if (response.status === 403) {
      throw new Error("GitHub API access forbidden. Check token permissions.");
    } else if (response.status === 404) {
      throw new Error(
        "Repository or workflow not found. Check repository visibility.",
      );
    }
...
<div id="auth-modal" class="modal" style="display: none;">
  <div class="modal-content">
    ...
    <p>
      To access deployment data, please provide a GitHub Personal Access Token.
    </p>
