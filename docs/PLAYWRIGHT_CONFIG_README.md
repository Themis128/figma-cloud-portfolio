...
- **Lambda Function URL**: `https://3qwsvw4rykz7ohbywemjeq5vxq0bekyw.lambda-url.us-east-1.on.aws/`
- Endpoint configured in `.env` as `PLAYWRIGHT_AUTOFIX_ENDPOINT` and `AUTOFIX_LAMBDA_URL`
- Playwright config loads `.env` via dotenv
- Reporter connects to Lambda for AI-powered suggestions
- Tests run in autofix mode (`pnpm test:e2e:autofix`) use Lambda for real-time fixes
- Function returns fix suggestions with confidence scores (≥90% = auto-fixable)
- Falls back to offline local analysis if endpoint unavailable
...
- ✅ **Updated Base URLs** matching current app architecture (frontend: 8082, backend: 3002)
