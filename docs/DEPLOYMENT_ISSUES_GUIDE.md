...
3. Mark server-side secrets (`RECAPTCHA_SECRET_KEY`, `HF_TOKEN`, `CAL_API_KEY`, `GITHUB_TOKEN`) as "Secret"

**How to Fix**:

- Add missing variables in Amplify Console
- Test contact form, chat, and booking after deployment
...
### Scenario 2: API Routes Not Working

**Cause**: Next.js build failed or Amplify not configured for SSR framework
**Solution**: Ensure Amplify app framework is set to "Next.js - SSR" and `amplify.yml` uses `.next` as baseDirectory

### Scenario 3: Environment Variables Not Working

**Cause**: Variables set in GitHub secrets instead of Amplify Console, or missing `NEXT_PUBLIC_` prefix for client-side vars
**Solution**: Move variables to Amplify Console environment variables. Client-side vars must have `NEXT_PUBLIC_` prefix
