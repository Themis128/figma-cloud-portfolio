...
```bash
# Option 1: Quick start with provided script (recommended)
./serve-dashboard.sh    # Linux/Mac
# or
serve-dashboard.bat     # Windows

# Option 2: Manual setup with http-server
npx http-server playwright-tests/ -p 8080 --cors

# Option 3: Open directly in browser (limited functionality due to CORS)
start playwright-tests\visual-progress.html
```

### **CORS Limitations & Solutions**

When opening the dashboard directly in a browser (file:// protocol), you may encounter CORS errors when trying to access test result files. This is a browser security restriction.
...
1. **Use a local web server** (recommended):
   ...
   ```bash
   npx http-server playwright-tests/ -p 8080
   ```

   Then visit: `http://localhost:8080/visual-progress.html`

2. **Install http-server globally** for easier access:

   ```bash
   npm install -g http-server
   http-server playwright-tests/ -p 8080
   ```
...
**What happens with CORS restrictions:**

- The dashboard will fall back to simulated progress data
- Real test results won't be accessible
- All visual features and animations still work
- Logs console functions normally
